...
int DoubleRep( int argc, char* argv[] )
{
  ...
  if( argc == 3 ){
    Ufloat u;
    u.d = atoT<float>( argv[2] );            // read input from command line
    Ullong ull = (Ullong)u.i;                // convert to 64-bit for PrintRep
    bitset<MAXBITS> bits( BitSetULL(ull) );  // bit representation
    cout << u.d << " = " << PrintRep( F_EXPBITS,F_MANBITS,bits ) << endl;
  } // argc == 2 (float requested)

  // Double Precision requested
  else{
    Udoub u;
    u.d = atoT<double>( argv[2] );           // read input from command line
    bitset<MAXBITS> bits( BitSetULL(u.i) );  // bit representation
    cout << u.d << " = " << PrintRep( D_EXPBITS,D_MANBITS,bits ) << endl;
